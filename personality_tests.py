...
import math
from utils import logger, get_response_json, avg, std,torch_gc
import re 
...
def build_character_agent(character_code, agent_type, agent_llm):
    ...
    if agent_type_args[0] == 'ChatHaruhi':
        ...
        character_agent = ChatHaruhi( role_from_hf = f'silk-road/ChatHaruhi-from-RoleLLM/{character_info[character_code]["agent"]["RoleLLM"]}', llm_name = agent_llm, embedding = 'bge_en',llm=llm)
...
def interview(character_agent, questionnaire, experimenter, questionnaire_prompts, language, query_style, nth_test, if_multiround = 0):
...
def assess(character_aliases, experimenter, questionnaire_results, questionnaire, questionnaire_metadata, eval_method, language, evaluator_llm, nth_test, agent_llm):
    ...
    for result in results:
        ...
        # result : {'dim': dim, 'r                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                esponses': [id2results[idx]], 'score': score, 'choice' (optional): choice}
        ...
        if eval_method == 'direct':
            ...
            for nth_test in range(max(repeat_times, 1)):			
                ...
                if not os.path.exists(interview_save_path):
                    ...
                    questionnaire_results = interview(character_agent, questionnaire, experimenter, 
                                                      questionnaire_metadata["prompts"], language, query_style, nth_test, if_multiround)
